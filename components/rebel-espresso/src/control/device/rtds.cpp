...
static constexpr float A[6] = {-242.02, 2.2228, 2.5859e-3, 4.8260e-6, 2.8183e-8, 1.5243e-10};
...
static double _rtd_to_celcius(double vref, double value, double rtd_nominal) {
  ...
  Rrtd /= RNominal;
  Rrtd *= 100.0;
  return A[0] + A[1] * Rrtd + A[2] * pow(Rrtd, 2) + A[3] * pow(Rrtd, 3) + A[4] * pow(Rrtd, 4) + A[5] * pow(Rrtd, 5);
}
