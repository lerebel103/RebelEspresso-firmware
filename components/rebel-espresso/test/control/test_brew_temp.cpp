...
TEST_CASE("[brew_temp:test_standby_duty_max]", "Ensure STANDBY cuts duty to zero") {
...
TEST_CASE("[brew_temp:test_error_conditions]", "Ensure process cuts power when conditions not met") {
    ...
    TEST_ASSERT_EQUAL(70, brew_temp_get_status().brew_temp_read_error_count);
...
TEST_CASE("[brew_temp:test_update_config_from_json]", "Ensures partial json updates work") {
    ...
    cJSON_AddBoolToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "enabled", true);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "pid.P", 4.1);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "pid.I", 0.5);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "pid.D", 60.4);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "pid.setpoint0", 99.9);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "max_damping_perc", 1.34);
    cJSON_AddNumberToObject(root, "cfg." BREW_TEMP_CFG_JSON_KEY "reset_time_sec", 34);
...
TEST_CASE("[brew_temp:test_nvs_reset_default]", "Test resetting NVS to defaults") {
    ...
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.P", 4.1);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.I", 0.5);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.D", 60.4);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.I_reset_sec", 16);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.I_reset_temp", 14);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.setpoint0", 125);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.setpoint1", 139);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.over_setpoint_perc", 25);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "pid.min_duty_band", 8);
    cJSON_AddBoolToObject(root, BREW_TEMP_CFG_JSON_KEY "enabled", true);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "max_damping_perc", 3.1);
    cJSON_AddNumberToObject(root, BREW_TEMP_CFG_JSON_KEY "reset_time_sec", 1);
...
TEST_CASE("[brew_temp:test_persit_stats]", "Test that stats are persisted ok") {
    ...
    brew_temp_delete();
    brew_temp_init(g_event_loop);
    TEST_ASSERT_EQUAL(0, brew_temp_get_status().brew_temp_read_error_count);
    ...
    nvram_store_get_u32(my_handle, KEY_BREW_TEMP_STATS_TEMP_ERROR, (uint32_t *) &stats.brew_temp_read_error_count,
                        (void *) &defaultVal);
...
TEST_CASE("[brew_temp:test_backoff_after_brew]", "Test damper off when brew started") {
    xEventGroupSetBits(status_event_group, POWER_ON_BIT);
    xEventGroupSetBits(status_event_group, BOILER_LEVEL_OK_BIT);
    brew_temp_init(g_event_loop);
    brew_temp_reset_cfg();
