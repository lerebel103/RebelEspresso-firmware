...
static esp_err_t _mount_data_partition() {
  ...
  esp_err_t err = esp_vfs_fat_spiflash_mount_rw_wl(DATA_MOUNT_POINT, DATA_PARTITION_LABEL, &mount_config, &s_wl_handle);

  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to mount FAT partition '%s': %s", DATA_PARTITION_LABEL, esp_err_to_name(err));
    return err;
  }
