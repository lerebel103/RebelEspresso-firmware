...
TEST_CASE("[brew_temp:test_standby_hbridge_off]", "Ensure STANDBY cuts off power to H-Bridge") {
    ...
    // and observe that it is dropped back to 0 forcefully when brew_temp is disabled
    // + duty is set to zero
...
TEST_CASE("[brew_temp:test_error_conditions]", "Ensure process cuts power when conditions not met") {
    brew_temp_init(g_event_loop);
    brew_temp_reset_stats();
    ...
    TEST_ASSERT_EQUAL(70, brew_temp_get_status().temp_read_error_count);

    brew_temp_delete();
}


TEST_CASE("[brew_temp:test_update_config_from_json]", "Ensures partial json updates work") {
    brew_temp_init(g_event_loop);
    brew_temp_reset_cfg();
    cJSON *root = cJSON_CreateObject();

    cJSON_AddNumberToObject(root, "cfg." BREW_CFG_JSON_KEY "pid.P", 4.1);
    cJSON_AddNumberToObject(root, "cfg." BREW_CFG_JSON_KEY "pid.I", 0.5);
    cJSON_AddNumberToObject(root, "cfg." BREW_CFG_JSON_KEY "pid.D", 60.4);
    cJSON_AddNumberToObject(root, "cfg." BREW_CFG_JSON_KEY "pid.setpoint0", 99.9);
    cJSON_AddNumberToObject(root, "cfg." BREW_CFG_JSON_KEY "hysteresis", 1.34);

    brew_temp_update_cfg(root);
    brew_temp_cfg_t cfg = brew_temp_get_cfg();
...
TEST_CASE("[brew_temp:test_nvs_load_save]", "Test load/save config works") {
    brew_temp_init(g_event_loop);

    brew_temp_cfg_t new_cfg;
    new_cfg.pid.P = 5.5;
    new_cfg.pid.I = 5.6;
    new_cfg.pid.D = 5.7;
...
TEST_CASE("[brew_temp:test_nvs_reset_default]", "Test resetting NVS to defaults") {
    // reset
    brew_temp_reset_cfg();
    brew_temp_init(g_event_loop);
    ...
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.P", 4.1);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.I", 0.5);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.D", 60.4);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.I_reset_sec", 16);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.I_reset_temp", 14);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.setpoint0", 125);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.setpoint1", 139);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.over_setpoint_perc", 25);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "pid.min_duty_band", 8);
    cJSON_AddNumberToObject(root, BREW_CFG_JSON_KEY "hysteresis", 3.1);
...
TEST_CASE("[brew_temp:test_brew_setpoint_inc]", "Test increment brew setpoint") {
    ...
    for (int i = 0; i < 10; i++) {
        expected += inc;
        TEST_ASSERT_EQUAL_DOUBLE(expected, brew_setpoint_inc(inc));

        // Now this must be stored in nvram
        brew_temp_delete();
        brew_temp_init(g_event_loop);
        TEST_ASSERT_EQUAL_DOUBLE(expected, brew_temp_get_cfg().pid.setpoints[0]);
    }

    // go out of bounds
    inc = 1000;
    for (int i = 0; i < 10; i++) {
        TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MAX, brew_setpoint_inc(inc));
        TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MAX, brew_temp_get_cfg().pid.setpoints[0]);
    }
    brew_temp_delete();
    brew_temp_init(g_event_loop);
    TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MAX, brew_temp_get_cfg().pid.setpoints[0]);
    ...
    inc = -1000;
    for (int i = 0; i < 10; i++) {
        TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MIN, brew_setpoint_inc(inc));
        TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MIN, brew_temp_get_cfg().pid.setpoints[0]);
    }
    brew_temp_delete();
    brew_temp_init(g_event_loop);
    TEST_ASSERT_EQUAL_DOUBLE(SETPOINT0_MIN, brew_temp_get_cfg().pid.setpoints[0]);
...
TEST_CASE("[brew_temp:test_brew_duty_ramp_up]", "Test duty when temp ramp up") {
    ...
    for (int i = 0; i < 150; i++) {
        ...
        if (data.temperature > 88) {
            ...
            TEST_ASSERT_EQUAL(expectedDuties[i], brew_temp_get_duty());
        }
    }

    // Over temp got triggered this many times
    TEST_ASSERT_EQUAL(17, brew_temp_get_status().temp_over_limit_count);

    brew_temp_delete();
}
*/

TEST_CASE("[brew_temp:test_persit_stats]", "Test that stats are persisted ok") {
    xEventGroupSetBits(status_event_group, POWER_ON_BIT);
    xEventGroupSetBits(status_event_group, BOILER_LEVEL_OK_BIT);
    brew_temp_init(g_event_loop);
    brew_temp_reset_stats();
    ...
    TEST_ASSERT_EQUAL(1, brew_temp_get_status().temp_read_error_count);

    // but it did not persist just yet
    brew_temp_delete();
    brew_temp_init(g_event_loop);
    TEST_ASSERT_EQUAL(0, brew_temp_get_status().temp_read_error_count);

    // Do it again, issue TICK, and it will take straight away
    brew_temp_process(1.3e6, data);
    TEST_ASSERT_EQUAL(1, brew_temp_get_status().temp_read_error_count);

    brew_temp_status_t stats;
    ...
    nvram_store_get_u32(my_handle, KEY_BREW_STATS_TEMP_ERROR, (uint32_t *) &stats.temp_read_error_count,
                        (void *) &defaultVal);
